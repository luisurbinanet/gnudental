...
namespace OpenDental{
	...
	public class FormRpProduction : System.Windows.Forms.Form{
		...
		private System.Windows.Forms.TextBox textToday;  //adjments
		...
		private void InitializeComponent()
		{
			...
			this.butOK.Location = new System.Drawing.Point(422, 266);
			...
			this.butAll.Location = new System.Drawing.Point(39, 295);
			...
			this.datePickerFrom.Location = new System.Drawing.Point(98, 10);
			this.datePickerFrom.Name = "datePickerFrom";
			this.datePickerFrom.ShowUpDown = true;
			this.datePickerFrom.Size = new System.Drawing.Size(105, 20);
			this.datePickerFrom.TabIndex = 36;
			// 
			// label2
			// 
			...
			this.datePickerTo.Location = new System.Drawing.Point(98, 40);
			this.datePickerTo.Name = "datePickerTo";
			this.datePickerTo.ShowUpDown = true;
			this.datePickerTo.Size = new System.Drawing.Size(105, 20);
			this.datePickerTo.TabIndex = 38;
			// 
			// panelMonth
			// 
			this.panelMonth.Controls.Add(this.label3);
			this.panelMonth.Controls.Add(this.datePickerFrom);
			this.panelMonth.Controls.Add(this.label2);
			this.panelMonth.Controls.Add(this.datePickerTo);
			this.panelMonth.Location = new System.Drawing.Point(290, 37);
			this.panelMonth.Name = "panelMonth";
			this.panelMonth.Size = new System.Drawing.Size(220, 69);
		...
		private void SetDates(){
			if(radioDaily.Checked){
				datePickerFrom.Value=new DateTime(DateTime.Today.Year,DateTime.Today.Month,1);
				datePickerTo.Value=new DateTime(DateTime.Today.Year,DateTime.Today.Month
					,DateTime.DaysInMonth(DateTime.Today.Year,DateTime.Today.Month));
			}
			else{//monthly (not in use yet)
				datePickerFrom.Value=new DateTime(DateTime.Today.Year,1,1);
				datePickerTo.Value=new DateTime(DateTime.Today.Year,12,31);
			}
		}
...
*/
			...
			Queries.CurReport.Query="SELECT procdate, SUM(procfee) FROM procedurelog WHERE "
				+"procdate >= '" + datePickerFrom.Value.ToString("yyyy-MM-dd")+"' "
				+"&& procdate <= '" + datePickerTo.Value.ToString("yyyy-MM-dd")+"' "
				+"&& procstatus = '2' "
				+whereProv
...
*/
        ...
        +"appointment.AptStatus = 4) && "//or stat=ASAP
				...
				+datePickerFrom.Value.ToString("yyyy-MM-dd")+"' "
				+"&& FROM_DAYS(TO_DAYS(appointment.AptDateTime)) <= '"
				+datePickerTo.Value.ToString("yyyy-MM-dd")+"' "
				+whereProv
...
*/
			...
			Queries.CurReport.Query= "SELECT paysplit.procdate,SUM(paysplit.splitamt) FROM paysplit "
				...
				+"&& paysplit.procdate >= '" + datePickerFrom.Value.ToString("yyyy-MM-dd")+"' "
				+"&& paysplit.procdate <= '" + datePickerTo.Value.ToString("yyyy-MM-dd")+"' "
				+whereProv
				+" GROUP BY paysplit.procdate UNION ALL SELECT claimpayment.checkdate,SUM(claimproc.inspayamt) "
				+"FROM claimpayment,claimproc WHERE "
				+"claimproc.claimpaymentnum = claimpayment.claimpaymentnum "
				+"&& claimpayment.checkdate >= '" + datePickerFrom.Value.ToString("yyyy-MM-dd")+"' "
				+"&& claimpayment.checkdate <= '" + datePickerTo.Value.ToString("yyyy-MM-dd")+"' "
				+whereProv2
				+" GROUP BY claimpayment.checkdate ORDER BY procdate";
			FormQuery2=new FormQuery();
			FormQuery2.IsReport=true;
...
*/ 
  		Queries.CurReport=new Report();
			...
			Queries.CurReport.Query="SELECT adjdate, SUM(adjamt) FROM adjustment WHERE "
				+"adjdate >= '" + datePickerFrom.Value.ToString("yyyy-MM-dd")+"' "
				+"&& adjdate <= '" + datePickerTo.Value.ToString("yyyy-MM-dd")+"' "
				+whereProv
...
//Now to fill Table Q from the 4 temp tables
			...
			double income;//just used below in the tablepay loop to combine patient and ins payments
			double adjust;
			DateTime[] dates=new DateTime[(datePickerTo.Value-datePickerFrom.Value).Days+1];
			...
			//for(int i=0;i<TableDate.Rows.Count;i++){//loop through data rows
				dates[i]=datePickerFrom.Value.AddDays(i);
          ...
          row[3]="0.00";
				...
				for(int j=0; j<TablePay.Rows.Count; j++)  {
				  if(dates[i]
						...
						income+=PIn.PDouble(TablePay.Rows[j][1].ToString());
					}																																						 
   			}
				...
				row[4]=income.ToString("F");
				row[5]=adjust.ToString("F");
				row[6]=(income+adjust).ToString("F");
				Queries.CurReport.ColTotal[4]+=income;
				Queries.CurReport.ColTotal[5]+=adjust;
				Queries.CurReport.ColTotal[6]+=income+adjust;
        if (row[4].ToString().Equals(""))
          ...
          row[6]="0.00";
			...
			//if(radioRange.Checked==true){
				Queries.CurReport.SubTitle[1]=datePickerFrom.Value.ToString("d")+" - "
					+datePickerTo.Value.ToString("d");
			//}
			//else{
			//	Queries.CurReport.SubTitle[1]=datePickerFrom.Value.ToString("d");
			//}
			...
			Queries.CurReport.ColCaption[6]="Net Income";
