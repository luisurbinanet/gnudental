...
namespace OpenDental{
	...
	public class FormScreenGroup : System.Windows.Forms.Form{
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(FormScreenGroup));
		...
		private void FormScreenGroup_Load(object sender, System.EventArgs e) {
			...
			if(IsNew){
				ScreenGroups.InsertCur();
			}
			FillGrid();
			if(Screens.List.Length>0){
				Screens.Cur=Screens.List[0];
				ScreenGroups.Cur.SGDate=Screens.Cur.ScreenDate;
				ScreenGroups.Cur.ProvName=Screens.Cur.ProvName;
				ScreenGroups.Cur.ProvNum=Screens.Cur.ProvNum;
				ScreenGroups.Cur.County=Screens.Cur.County;
				ScreenGroups.Cur.GradeSchool=Screens.Cur.GradeSchool;
				ScreenGroups.Cur.PlaceService=Screens.Cur.PlaceService;
			}
			textScreenDate.Text=ScreenGroups.Cur.SGDate.ToShortDateString();
			textDescription.Text=ScreenGroups.Cur.Description;
			textProvName.Text=ScreenGroups.Cur.ProvName;//has to be filled before provnum
			for(int i=0;i<Providers.List.Length;i++){
				comboProv.Items.Add(Providers.List[i].Abbr);
				if(ScreenGroups.Cur.ProvNum==Providers.List[i].ProvNum){
					comboProv.SelectedIndex=i;
				}
			}
			comboCounty.Items.AddRange(Counties.ListNames);
			if(ScreenGroups.Cur.County==null)
				ScreenGroups.Cur.County="";//prevents the next line from crashing
			comboCounty.SelectedIndex=comboCounty.Items.IndexOf(ScreenGroups.Cur.County);//"" etc OK
			comboGradeSchool.Items.AddRange(Schools.ListNames);
			if(ScreenGroups.Cur.GradeSchool==null)
				ScreenGroups.Cur.GradeSchool="";//prevents the next line from crashing
			comboGradeSchool.SelectedIndex=comboGradeSchool.Items.IndexOf(ScreenGroups.Cur.GradeSchool);//"" etc OK
			comboPlaceService.Items.AddRange(Enum.GetNames(typeof(PlaceOfService)));
			comboPlaceService.SelectedIndex=(int)ScreenGroups.Cur.PlaceService;
		}
		...
		private void listMain_DoubleClick(object sender, System.EventArgs e) {
			...
			Screens.Cur=Screens.List[listMain.SelectedIndices[0]];
			FormSE.ShowDialog();
		...
		private void butAdd_Click(object sender, System.EventArgs e) {
			...
			FormSE.IsNew=true;
			if(Screens.List.Length==0){
				Screens.Cur=new Screen();
				Screens.Cur.ScreenGroupOrder=1;
			}
			else{
				Screens.Cur=Screens.List[Screens.List.Length-1];//'remembers' the last entry
		...
		private void butDelete_Click(object sender, System.EventArgs e) {
			...
			for(int i=0;i<listMain.SelectedIndices.Count;i++){
				Screens.Cur=Screens.List[listMain.SelectedIndices[i]];
				Screens.DeleteCur();
			}
			FillGrid();
		}
		...
		private void butOK_Click(object sender, System.EventArgs e) {
			...
			ScreenGroups.Cur.SGDate=PIn.PDate(textScreenDate.Text);
			ScreenGroups.Cur.Description=textDescription.Text;
			ScreenGroups.Cur.ProvName=textProvName.Text;
			ScreenGroups.Cur.ProvNum=comboProv.SelectedIndex+1;//this works for -1 also.
			if(comboCounty.SelectedIndex==-1)
				ScreenGroups.Cur.County="";
			else
				ScreenGroups.Cur.County=comboCounty.SelectedItem.ToString();
			if(comboGradeSchool.SelectedIndex==-1)
				ScreenGroups.Cur.GradeSchool="";
			else
				ScreenGroups.Cur.GradeSchool=comboGradeSchool.SelectedItem.ToString();
			ScreenGroups.Cur.PlaceService=(PlaceOfService)comboPlaceService.SelectedIndex;
			ScreenGroups.UpdateCur();
			Screens.UpdateForGroup();
