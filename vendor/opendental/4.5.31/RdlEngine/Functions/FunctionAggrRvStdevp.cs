...
{
	/// <summary>
	/// <p>Aggregate function: RunningValue stdevp
	/// <p>
	///	
	/// </summary>
	[Serializable]
	internal class FunctionAggrRvStdevp : FunctionAggr, IExpr
	{
		private double _sum;		// 
		...
		/// </summary>
		public FunctionAggrRvStdevp(IExpr e, object scp):base(e, scp) 
		{
			_sum = _sum2 = 0;
			_count = -1;
		}
		...
		public double EvaluateDouble(Row row)
		{
			...
			double result;
			if (_count > 0)
				result = Math.Sqrt((_count * _sum2 - _sum*_sum) / (_count * _count));
			else
				result = double.NaN;
