...
namespace OpenDental{
	...
	public class FormBackup : System.Windows.Forms.Form{
		...
		private void butRestore_Click(object sender, System.EventArgs e) {
			...
			Cursor=Cursors.WaitCursor;
			//make sure it's a file we created:
			using(StreamReader sr=new StreamReader(openDlg.FileName)){
					...
					&& buffer[4]==';')
				{
					...
					MsgBox.Show(this,"Invalid file format.  You can only use files that were originally generated by this program.");
					Cursor=Cursors.Default;
					return;
				}
			}
			...
			using(BinaryReader br=new BinaryReader(fs)){
				char onechar;
				for(int i=0;i<5;i++){
					onechar=br.ReadChar();//gets rid of "v3.5;"
				}
				string tempFile=Prefs.GetString("DocPath")+"TempDumpFile.txt";//for one table at a time
				string tableName="";
				...
				while(true){
					...
					if(onechar=='E' && tableData.Length>10 
						&& tableData.ToString().Substring(tableData.Length-11,11)=="CREATE TABL")
					{
