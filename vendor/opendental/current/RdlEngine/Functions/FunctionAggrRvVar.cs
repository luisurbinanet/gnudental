...
{
	/// <summary>
	/// <p>Aggregate function: RunningValue var
	/// <p>
	///	
	/// </summary>
	[Serializable]
	internal class FunctionAggrRvVar : FunctionAggr, IExpr
	{
		private double _sum;		// 
		...
		/// </summary>
		public FunctionAggrRvVar(IExpr e, object scp):base(e, scp) 
		{
			_sum = _sum2 = 0;
			_count = -1;
		}
		...
		public double EvaluateDouble(Row row)
		{
			...
			double result;
			if (_count > 1)
				result = (_count * _sum2 - _sum*_sum) / (_count * (_count-1));
			else		
				result = double.NaN;
