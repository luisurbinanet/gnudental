...
namespace OpenDental{
	...
	public class FormLetterMergeEdit : System.Windows.Forms.Form{
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(FormLetterMergeEdit));
		...
		private void FormLetterMergeEdit_Load(object sender, System.EventArgs e) {
			...
			for(int i=0;i<Defs.Short[(int)DefCat.LetterMergeCats].Length;i++){
				comboCategory.Items.Add(Defs.Short[(int)DefCat.LetterMergeCats][i].ItemName);
				if(LetterMergeCur.Category==Defs.Short[(int)DefCat.LetterMergeCats][i].DefNum){
