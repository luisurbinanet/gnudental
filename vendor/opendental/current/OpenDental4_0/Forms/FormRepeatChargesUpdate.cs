...
namespace OpenDental{
	...
	public class FormRepeatChargesUpdate : System.Windows.Forms.Form{
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(FormRepeatChargesUpdate));
			...
			this.textBox1.Text = @"This will add completed procedures to each account that has a repeating charge set up.   The date of the procedure will be based on the settings in the repeating charge.

If it has been more than a month since this was run, then more than one procedure might be added to some accounts.  You can safely run this as often as you want, because it will not add more than one procedure per month.  It will also not go back more than 3 months.
