...
namespace fyiReporting.RDL
{
	...
	internal class ChartBar: ChartBase
	{
		...
		internal ChartBar(Chart c, MatrixCellEntry[,] m) : base(c, m)
		{
		}
		...
		override internal void Draw()
		{
			...
			using (Graphics g = Graphics.FromImage(_bm))
			{
				...
				if (ChartDefn.Subtype == ChartSubTypeEnum.Stacked)
					DrawPlotAreaStacked(g, max);
				else if (ChartDefn.Subtype == ChartSubTypeEnum.PercentStacked)
					DrawPlotAreaPercentStacked(g);
				else
					DrawPlotAreaPlain(g, max);

				DrawLegend(g, false, false);		// after the plot is drawn
			}
		}
 
		void DrawPlotAreaPercentStacked(Graphics g)
		{
			...
			for (int iRow = 1; iRow <= CategoryCount; iRow++)
			{
				...
				for (int iCol = 1; iCol <= SeriesCount; iCol++)
				{
					...
					DrawColumnBar(g, SeriesBrush[iCol-1], rect, iRow, iCol);
					saveX = x;
				}
			}
		...
		void DrawPlotAreaPlain(Graphics g, double max)
		{
			...
			for (int iRow=1; iRow <= CategoryCount; iRow++)
			{
				...
				for (int iCol=1; iCol <= SeriesCount; iCol++)
				{
					...
					DrawColumnBar(g, SeriesBrush[iCol-1], 
		...
		void DrawPlotAreaStacked(Graphics g, double max)
		{
			int barsNeeded = CategoryCount; 
			int gapsNeeded = CategoryCount * 2;
		...
		Size CategoryAxisSize(Graphics g)
		{
		...
		// DrawCategoryAxis 
		void DrawCategoryAxis(Graphics g, System.Drawing.Rectangle rect)
		{
			if (this.ChartDefn.CategoryAxis == null)
				return;
			Axis a = this.ChartDefn.CategoryAxis.Axis;
			...
			DrawTitle(g, a.Title, new System.Drawing.Rectangle(rect.Left, rect.Top, tSize.Width, rect.Height));
		...
		void DrawColumnBar(Graphics g, Brush brush, System.Drawing.Rectangle rect, int iRow, int iCol)
		{
			...
			{
				DrawDataPoint(g, rect, iRow, iCol);
			}
			else
			{
				Point p;
				p = new Point(rect.Right, rect.Top);
				DrawDataPoint(g, p, iRow, iCol);
			}
		...
		protected void DrawValueAxis(Graphics g, double min, double max, System.Drawing.Rectangle rect, int plotTop, int plotBottom)
		{
			...
			for (int i=0; i < 11; i++)	// TODO: hard coding 11 is too simplistic 
			{
				...
				{
					size = s.MeasureString(g, v, TypeCode.Double, null, int.MaxValue);
					...
					s.DrawString(g, v, TypeCode.Double, null, vRect);
				}
				else
				{
					size = Style.MeasureStringDefaults(g, v, TypeCode.Double, null, int.MaxValue);
					System.Drawing.Rectangle vRect = 
						new System.Drawing.Rectangle(rect.Left + x - size.Width/2, rect.Top+tickSize, size.Width, size.Height);
				...
				DrawValueAxisGrid(g, a.MajorGridLines, new Point(rect.Left + x, plotTop), new Point(rect.Left + x, plotBottom));
				DrawValueAxisTick(g, true, a.MajorTickMarks, a.MajorGridLines, new Point(rect.Left + x, plotBottom ));

				v += incr;
			}

			// Draw the end points of the major grid lines
			DrawValueAxisGrid(g, a.MajorGridLines, new Point(rect.Left, plotTop), new Point(rect.Left, plotBottom));
			DrawValueAxisTick(g, true, a.MajorTickMarks, a.MajorGridLines, new Point(rect.Left, plotBottom));
			DrawValueAxisGrid(g, a.MajorGridLines, new Point(rect.Right, plotTop), new Point(rect.Right, plotBottom));
			DrawValueAxisTick(g, true, a.MajorTickMarks, a.MajorGridLines, new Point(rect.Right, plotBottom));

			Size tSize = DrawTitleMeasure(g, a.Title);
			DrawTitle(g, a.Title, new System.Drawing.Rectangle(rect.Left, rect.Top+maxValueHeight+tickSize, rect.Width, tSize.Height));

			return;
		}

		protected void DrawValueAxisGrid(Graphics g, ChartGridLines gl, Point s, Point e)
		{
			if (gl == null || !gl.ShowGridLines)
				return;

			if (gl.Style != null)
				gl.Style.DrawStyleLine(g, null, s, e);
			else
		...
		protected void DrawValueAxisTick(Graphics g, bool bMajor, AxisTickMarksEnum tickType, ChartGridLines gl, Point p)
		{
			...
			if (style != null)
				style.DrawStyleLine(g, null, s, e);
			else
				g.DrawLine(Pens.Black, s, e);
		...
		protected Size ValueAxisSize(Graphics g, double min, double max)
		{
			Size size=Size.Empty;
			...
			{
				minSize = a.Style.MeasureString(g, min, TypeCode.Double, null, int.MaxValue);
				maxSize = a.Style.MeasureString(g, max, TypeCode.Double, null, int.MaxValue);
			}
			else
			{
				minSize = Style.MeasureStringDefaults(g, min, TypeCode.Double, null, int.MaxValue);
				maxSize = Style.MeasureStringDefaults(g, max, TypeCode.Double, null, int.MaxValue);
			}
			// Choose the largest
			...
			Size titleSize = DrawTitleMeasure(g, a.Title);
