	...
	public class FormRpAdjSheet : System.Windows.Forms.Form{
		...
		private void butOK_Click(object sender, System.EventArgs e) {
			...
			Queries.CurReport.Query="SELECT adjustment.adjdate,"
			  ...
			  +"&& patient.patnum=adjustment.patnum && ";
			if(radioRange.Checked==true){
				Queries.CurReport.Query
					+="adjustment.adjdate >= '" + date1.SelectionStart.ToString("yyyy-MM-dd")+"' "
					+"&& adjustment.adjdate <= '" + date2.SelectionStart.ToString("yyyy-MM-dd")+"'";
			}
			else{
				Queries.CurReport.Query
					+="adjustment.adjdate = '" + date1.SelectionStart.ToString("yyyy-MM-dd")+"'";
			}
			Queries.CurReport.Query += " ORDER BY adjustment.adjdate DESC";
