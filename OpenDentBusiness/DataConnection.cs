...
namespace OpenDentBusiness{
	...
	public class DataConnection{//
		...
		private static int GetDefaultPortNum(){
			switch(DBtype){
		...
		public static string BuildSimpleConnectionString(DatabaseType pDbType,string pServer,string pDatabase,string pUserID,string pPassword){
			string connectStr="";
			string pPort=GetDefaultPortNum().ToString();
		...
		public void SetDb(string server, string database,string user, string password, string userLow, string passLow, DatabaseType dbtype){
			DBtype=dbtype;
			string connectStr=BuildSimpleConnectionString(server,database,user,password);
			string connectStrLow="";
			if(userLow!=""){
				connectStrLow=BuildSimpleConnectionString(server,database,userLow,passLow);
			}
		...
		public DataConnection(string serverName, string database, string mysqlUser, string mysqlPass,DatabaseType dtype){
			string connectStr=BuildSimpleConnectionString(dtype,serverName,database,mysqlUser,mysqlPass);
				...
				try{
					...
					if(getInsertID){
						try{
							...
							Logger.openlog.curSev=Logger.Severity.ERROR;
							Logger.openlog.LogMB("Oracle SQL Error: "+cmdOr.CommandText+"\r\n"+"Exception: "+e.ToString());
							innerException = e;//continue to pass the exception one level up.
						}
					}
